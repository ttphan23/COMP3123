...
ANSWER: 
The purpose of 'express.Router()' is used to create seperate route handlers in different files. This is to help organize code better and to manage the project easier so that not everything is packed inside one file like 'index.js'.   
...
ANSWER:
Error handling makes sure the app responds with a clear message when something goes wrong (like missing files or server errors). What this looks like is that with error handling, instead of the application just crashing, it would instead send a message like "Server Error - please try again" or even "Error: Route not found" so the user has a better understanding of the situation. 
...
ANSWER:
It is useful in production environments because the hosting service gives a port in "process.env.PORT". Locally, it defaults to 8081. This makes the app work both locally and in production without code changes.
